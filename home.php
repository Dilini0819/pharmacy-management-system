...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <div class="container-fluid"> <!-- bootstrap full width container-->
      <div class="container">  <!-- bootstrap fixed width container -->
        <!-- header section -->
        <?php
        ...
        <!-- form content -->
        <div class="row"> <!--bootsrap row for grid layout-->
          <div class="row col col-xs-8 col-sm-8 col-md-8 col-lg-8">

            <?php
              function createSection1($location, $title, $table) { //create function
                require 'config.php'; //database connection

                //out of stoke inna gana select krnw
                $query = "SELECT * FROM $table";
                if($title == "Out of Stock")
                  $query = "SELECT * FROM $table WHERE QUANTITY = 0";

                $result = mysqli_query($conn, $query);
                //execute a sql query on a database = $conn(connection of database) $query(above query)

                $count = mysqli_num_rows($result); 
                //show number of row return by the query 



                //expired wela thiyn gana select krnw
              ...
              //<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4" style="padding: 10px"> 
              // creata bootsrap grid column phpne tab,desktop
